...
size â€” which is the point of that section.
...
Corollary 6, and the three-state example in full â€” r = 1.5, lambda_12 = lambda_21 =
1.5, lambda_31 = lambda_32 = 0.75, conflict allocation q = (0.4, 0.4, 0.1), surplus
C = 0.1 â€” confirming that x = (0.4, 0.4, 0.2) is accepted by all three, that states
1 and 2 are exactly indifferent while state 3 strictly gains, and that the set of
admissible unit destinations is exactly {3}. Section 8's enlargement claim is
checked as well: a salient entrant never lowers rho(Lambda).
